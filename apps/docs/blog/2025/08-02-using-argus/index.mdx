...
# How to use Argus with Muninn

In `v1.32.0` we added the endpoint `/api/info` which returns the current version of Muninn and can therefore be used in Argus to watch for updates.
...
Replace the `<secret>` with the apiKey you created and the deployed version url with the url of your Muninn instance.