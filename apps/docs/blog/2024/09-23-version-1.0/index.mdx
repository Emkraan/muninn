...
# Muninn 1.0 - complete rewrite of all systems
We proudly present to you - Muninn ``1.0``.
...
See [this guide on how to import your previous Muninn dashboard](/docs/getting-started/after-the-installation#importing-a-zip-from-version-before-100)
...
- **Improved performance**: We have upgraded the underlying framework of Muninn and take advantage
...
- Mounts
  ...
  - Moved ``/data`` to ``/appdata`` and restructured contents (not backwards compatible!) -> Create a backup before migration of the entire directory to copy data to new Muninn
- Widgets:
  - Calendar:
...
- Database:
  - Changed schema of the database to a completely new schema. They are not backwards compatible. Please export the data from old Muninn in the UI and import it in ``1.0``
  - Removed all usage of JSON files to store data. Import JSON files in Muninn ``1.0`` to import your dashboards.
...
- Platform specific breaking changes:
  ...
  - Unraid
    - The Muninn app must be reinstalled. The existing directories on your disk are not compatible with Muninn 1.0. Export your old data in < `` 1.0`` and install using different file system locations.
  - Kubernetes
