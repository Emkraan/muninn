...
import manageExportOldMuninn from "./img/old-homarr-export-data.png";
import manageImportOldMuninnDropzone from "./img/old-homarr-import-dropzone.png";
import manageImportOldMuninnChooseData from "./img/old-homarr-import-choose-imported-data.png";
import manageImportDecryptionToken from "./img/old-homarr-import-decryption-token.png";

:::tip

Read our [glossary](/docs/getting-started/glossary) if you are new to Muninn.

:::

Congratulations on installing Muninn. Now it's time to get yourself familiar with the concepts of Muninn and how to use it.

## Onboarding

The onboard will show the first time you start Muninn.
This process will help you with your first steps and adding some initial data, such as the administrator account.

:::note

If you see this screen repeatitly after you restarted the Muninn container, your Docker mounting paths may be missing,
...
If the credentials provider was selected, you'll be prompted to create an admin user for Muninn.
This user cannot be deleted and has full control over Muninn and all of its boards.
...
This group will be used to synchronize users from your provider to Muninn.
When a user logs in through the provider, any of their groups that match groups in Muninn will be assigned to the user.
...
Next, you can configure some basic settings for Muninn. We recommend to leave all settings at default for most users.
...
If Docker is available (socket mounted), Muninn automatically detects running containers and matches them to known integration types.
...
This is recommended when you used Homarr before 1.0 and want to re-import your data.
It is important that you first migrate correctly using [Homarr's list of breaking changes for 1.0](https://homarr.dev/blog/2024/09/23/version-1.0).

<img
  src={manageExportOldMuninn}
...
In Muninn, you organize your apps and widgets on boards.
...
Muninn ships with over 11K icons - so you can just search by entering a search text instead.
...
This is expected, because you haven't told Muninn what app it should show in this item.
...
This helps you to explore the capabilities of Muninn before you build your board in a suboptimal way.
...
An integration is a connection between Muninn and a third party application - usually something that you run and deploy yourself.
...
Depending on your networking setup and applications, the connection from Muninn might also get blocked by firewalls,
...
After completing the onboarding, Muninn provides interactive guided tours to help you discover key features.

### Management tour

When you first visit the management area (`/manage`), Muninn walks you through the main pages: boards, apps, integrations, and users (admin only).
...
- Integrate with Muninn with your own application using [the API](/docs/management/api/)

:::danger

The Muninn Team will never ask for your credentials. Do not send people your credentials, as they manage the access to your applications and can cause harm to your device if they are abused.
