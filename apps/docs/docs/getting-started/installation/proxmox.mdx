...
This guide covers how to install Muninn inside of a linux-container in ProxmoxVE, using a fully automated approach.
...
After you've installed Muninn, you can find a new LXC "homarr".
...
Some Muninn features require setting environment variables. When installing Muninn on Proxmox using the community-scripts installer, these can be configured in the file /opt/homarr.env. This path is also documented on the community-scripts website as the configuration path.
...
- If you are looking to report a bug, vulnerability or request a new feature, use https://github.com/Emkraan/muninn . You can ask for help on our Discord.

# How to find logs
Muninn runs under a system service called `homarr`, so getting the logs is a simple as using the following command inside the LXC (Container) Shell

```bash
systemctl status homarr
```

As Muninn logs really a log, it sometimes feels like the log is endless till you get the bottom, so a command like this only shows the latest 100 lines:
