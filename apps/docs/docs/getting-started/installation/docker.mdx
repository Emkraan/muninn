...
Finally, run ``docker compose up -d`` in the same directory. This will start the Muninn container in the background.
...
- Stop Muninn using ``docker compose down``
- Pull the newest image of Muninn using ``docker compose pull``
- Start Muninn again using ``docker compose up -d`` (``-d`` for detached mode - start in background)
- Delete the old image using ``docker image prune`` (*Warning: this also removes you other unused images - not just Muninn*)
...
- Prune unused Docker images using ``docker image prune`` (*Warning: this also removes you other unused images - not just Muninn*)
- (Optional): Prune any network or volumes if you use any.
- (Optional): Remove Muninn from your reverse proxy, VPNs or tunnels if you use any.
...
To install Muninn using Docker, simply run the following command.
...
docker run  \
  ...
  -v <your-path>/muninn/appdata:/appdata \
  -e SECRET_ENCRYPTION_KEY='your_64_character_hex_string' \
  -d ghcr.io/emkraan/muninn:latest
```
...
### Updating
To update Muninn, you must remove your container first. Make sure that you've mounted your data and that you have access to it, so your configuration doesn't get lost.

1. Run ``docker rm muninn`` to remove the container.
2. Pull the latest Muninn image ``docker pull ghcr.io/emkraan/muninn:latest``.
3. Re-run the command you used to install Muninn.
...
1. Obtain the container ID of your Muninn container using ``docker ps | grep muninn``.
2. Delete the container using ``docker rm <your-container-id>``
3. Prune unused Docker images using ``docker image prune`` (*Warning: this also removes you other unused images - not just Muninn*)
4. (Optional): Prune any network or volumes if you use any.
5. (Optional): Remove Muninn from your reverse proxy, VPNs or tunnels if you use any.
