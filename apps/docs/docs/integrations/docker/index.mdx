...
Otherwise, you'll need to mount the [docker socket](https://docs.docker.com/engine/security/protect-access/) to the Muninn container,
so Muninn can interact with the Docker service.
...
### Managing your Docker containers within Muninn
Muninn allows you to interact with Docker containers running on your system.
...
- **Add to Muninn** - create a bookmark app from the container
...
Muninn also lets you add apps from containers. You can simply select all containers you want to add an app for and then click the `Add to Muninn` button.
This will open a dialog where you can specify the web addresses for all of them:

![Dialog for addding apps to Muninn](img/add-to-muninn-dialog.png)

Once you've changed the web addresses, click the `Add` button to add the apps to Muninn.
They are then available in the app list and can be used as normally added apps.

![Apps added from containers](img/add-to-muninn-apps.png)

### Configuration

Currently Muninn supports one label for configuration of the containers:
...
As an example, a thread actor could abuse Muninn use the socket to start, stop or delete containers on your system.
...
Muninn may behave in unexpected ways when you use proxies.

#### Permissions
Muninn needs the following permissions from the Docker API:
- Containers/Start
- Containers/Stop
- Containers/Restart
- Containers/Remove
...
Refer: https://github.com/Emkraan/muninn

You will also need to add Muninn to the network of your socket proxy. You can do it like this:
...
2. Add the network to the muninn service in the compose file:
```yaml
networks:
  - socket-proxy
```

3. Finally, if it is present, remove the default docker socket connection under `volumes` in the muninn service.
