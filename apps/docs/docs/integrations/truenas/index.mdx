...
<Alert type="info" title="Use the HTTPS address">
    TrueNAS only serves its API over HTTPS, so enter the <code>https://</code> URL of your TrueNAS web interface. If it uses a self-signed certificate (the default), add it under [Management &gt; Tools &gt; Certificates](/docs/management/certificates) so Muninn can establish the connection.
</Alert>
