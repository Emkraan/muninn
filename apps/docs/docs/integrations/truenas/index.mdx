...
<Alert type="info" title="Use the HTTPS address">
    TrueNAS only serves its API over HTTPS, so enter the <code>https://</code> URL of your TrueNAS web interface. If it uses a self-signed certificate (the default), add it under <a href="/docs/management/certificates">Management &gt; Tools &gt; Certificates</a> so Muninn can establish the connection.
</Alert>
...
<IntegrationSecrets secrets={[
    {
        ...
        steps: [
            ...
            "Click \"Add\", give the key a name such as \"Muninn\", and confirm.",
            "Copy the generated key immediately - TrueNAS only shows it once - and paste it as the API Key when creating the integration in Muninn.",
        ],
    },
    {
        ...
        steps: [
            ...
            "Grant the user administrative access so Muninn can read system, pool, and reporting data.",
            "Save the user and create the integration in Muninn with these credentials.",
        ],
