...
Use the root URL of your Coolify instance, for example `https://coolify.example.com`. Muninn calls Coolify endpoints under `/api` and `/api/v1` automatically.
:::
...
<IntegrationSecrets secrets={[{
  credentials: ['apiKey'],
  steps: [
    'Open your Coolify instance and make sure you are in the team you want Muninn to monitor.',
    ...
    'Paste the token into the API Key field in Muninn.'
  ]
}]} />
