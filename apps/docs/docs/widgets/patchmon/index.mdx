...
PatchMon does not render a built-in widget title. To add a label on your board, open **Edit item → Advanced options → Title**, the same optional overlay badge used by other Muninn widgets.

### Authentication

:::note
This widget requires a **signed-in Muninn session**. Anonymous visitors cannot load PatchMon data, even when the board is [public](/docs/management/boards#access-control). Placing this widget on a public board does not expose patch or security statistics to guests.
