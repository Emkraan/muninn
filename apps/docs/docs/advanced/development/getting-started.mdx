...
This guide will help you set up your development environment to start contributing to muninn.
...
3. Change directory to the project folder with `cd muninn`.

4. Install the dependencies with `pnpm install`.

5. Start redis by running `pnpm run docker:dev`. This will run a docker-compose file with a redis service.

6. Copy the `.env.example` file to `.env` and replace `DB_URL` with the full path you want to use for the sqlite database file. For example `DB_URL='C:\\Dev\\muninn\\packages\\db\\db.sqlite'`
...
- `docker build -t muninn .`: Build a docker image for the project.
- `docker run -p 7575:7575 -e SECRET_ENCRYPTION_KEY='your_64_character_hex_string' muninn:latest`: Run the created docker image.