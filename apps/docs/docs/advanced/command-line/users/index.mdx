...
Use "muninn users [command] --help" for more information about a command.
```
...
- Requires a password (which will be encrypted by Muninn)

```bash
Update the password for a user (credentials provider only)

Usage:
  muninn users update-password [flags]
