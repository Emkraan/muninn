...
Muninn is trying to communicate to your apps via the integrations.
It usually doesn't matter if Muninn is running on ``http`` or ``https``.
Your apps have a self-signed certificate - Muninn will recognize that the certificate was signed by an unknown authority and requests will be blocked.
...
## Securing Muninn with Traefik

Copying the configuration straight from the docker-compose file won't work if you are running Muninn behind Traefik, such as a Portainer setup, or docker-swarm. In that case, you should use the following slightly modified configuration:
```yaml
version: '3'
services:
  muninn:
    ...
    labels:
      traefik.enable: true
      traefik.http.routers.muninn.rule: Host(`your.internal.dns.address.here.com`)
      traefik.http.routers.muninn.entrypoints: websecure
      traefik.http.routers.muninn-secure.app: muninn
      traefik.http.apps.muninn.loadbalancer.server.port: 7575
...
```
:::important
Muninn needs to be on the same network so you will need to add the following property to your muninn install:
```yaml
muninn:
  ...
  networks:
