...
A user may also have a "home board" which they will see at the root path (e.g. my-muninn.com or my-muninn.com/board).
Other boards can be viewed by direct links (e.g. my-muninn.com/board/smart-home) or via the management pages.
...
### Column count
Muninn has a complex drag and drop system that can be used from the browser.
...
This field is optional and Muninn will fall back to "Muninn" if nothing is set.
...
Muninn will fall back to the board name and a suffix if nothing is set.

#### Logo image URL
This is the URL that is used for the image at the top left.
Muninn will fall back to the Muninn logo if nothing is set.
...
Muninn can display a background image behind apps and widgets.
...
The home board is the dashboard, that will be shown at the root path of Muninn or if no board name was passed in the URL.
All the following paths will lead to the home board:
- ``my-muninn.com``
- ``my-muninn.com/boards``
