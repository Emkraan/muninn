...
Each person accessing Muninn is a user. Using the users management you can configure who has access to certain resources.
...
Muninn supports [single sign on for easier and seamless authentication](/docs/advanced/single-sign-on) (e.g. Authelia).
...
By default, Muninn will choose English as the default language. Here you can override the language for this specific user.
...
It must also meet the password requirements of Muninn.
...
Muninn will prompt you for a group name.
...
#### Special groups
Currently one special group exists. This group is `everyone`. It automatically adds every user that is known to Muninn to its members. Neither its name nor its members can be changed.
