...
### Url
This must be the base URL of the application where Muninn should connect to.
...
To improve security, they will never be passed to the client and requests to the integrations are performed from the host where Muninn is running on.

### Linking applications

During the creation of an integration you can choose to add / link an application to the integration.
If you want to add one, you can simply specify the app url which will be used for opening the application from Muninn.
...
This is to ensure that you minimize the amount of broken applications on your Muninn instance.
...
3. Check whether you followed the Muninn documentation and didn't skip any steps.
...
4. Check Muninn's logs for more information. As we have no control over breaking changes in upstream APIs and third party software,
they can break without notice in unexpected ways which can also lead to errors within Muninn. In such case, these occurrences must be reported
and fixed within the source code of Muninn.
...
If you are using a self-signed certificate you'll need to add it to the [trusted certificates](../certificates) in Muninn.
