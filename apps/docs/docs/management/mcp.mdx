...
Muninn exposes a [Model Context Protocol](https://modelcontextprotocol.io) server at `/api/mcp/mcp`, enabling AI assistants like Claude, Cursor, VS Code Copilot, and Claude Code to interact with your Muninn instance using natural language.

With MCP, you can ask your AI assistant to manage apps, integrations, users, boards, Docker containers, media servers, DNS settings, and more - all through Muninn's existing tRPC API, now exposed as 50+ AI-callable tools.
...
| ------------------- | ------------------------------------------------------------ | ------------------------------ |
| **Streamable HTTP** | Cursor, Claude Desktop, VS Code Copilot, Claude Code         | Direct HTTP access to Muninn   |
...
```json
{
  "mcpServers": {
    "muninn": {
      "url": "https://your-muninn-instance.com/api/mcp/mcp",
      "headers": {
        "ApiKey": "<your-api-key>"
      }
    }
...
| Claude Code     | `.claude/mcp.json` (or `claude mcp add muninn`) |
  ...
  "mcpServers": {
    "muninn": {
      ...
      "args": [
        "-y",
        "mcp-remote",
        "https://your-muninn-instance.com/api/mcp/mcp",
        "--header",
        "ApiKey:<your-api-key>"
      ]
    }
...
| **OAuth 2.1 + PKCE** | Full OAuth flow with dynamic client registration      | MCP clients with OAuth support (e.g. `opencode mcp auth muninn`) |
...
Muninn exposes 50+ tools organized by namespace. Each tool is either a **query** (GET, read-only) or **mutation** (POST, write action).
...
The tools table on the MCP tab is dynamically generated - it always reflects the latest available procedures from your Muninn instance.
...
- **"Show me all apps on my Muninn dashboard"** - AI lists all apps with their names and URLs
...
- **"How many users are on my Muninn instance?"** - AI lists all users (admin only)
