...
# Settings
The settings page has a few options that will be applied to the entire Muninn application.
They may require a full page reload for changes to apply.

## Analytics
By default, Muninn is collecting anonymous analytics to identify issues and prioritize bugfixes and improvements.
...
We understand that you might not want Muninn to communicate with our services.
...
If you are paranoid, you can permanently block the analytics endpoint from your DNS / firewall.
...
Muninn is requesting the crawlers to not index Muninn instances by default for this reason.
If you use Muninn as a public website and want it to be searchable on these search engines, you can disable these settings.
Please note that Muninn doesn't have control over what the crawlers do; it is a mere request to them to not perform these actions but will not guarantee that your website doesn't land on some index.

### No index
This will ask crawlers to not index your Muninn instance on any indexes.
...
You can set a global mobile board. It will be shown instead of the global home board on mobile devices that can be detected by Muninn. (Using the User-Agent header).
