...
Muninn provides a built-in backup and restore system for **SQLite** instances. You can export your entire database - including apps, integrations, boards, users, and encryption keys - as a ZIP archive, and restore it on the same or a different Muninn instance.
...
| `metadata.json`      | Backup metadata: Muninn version, creation timestamp, encryption key |
...
Before restoring, Muninn uses **sql.js WASM** to analyze the backup directly in your browser. You'll see:
...
You can restore a backup from one Muninn instance onto another. Muninn handles:
...
- **Version compatibility** - Backups include the Muninn version they were created with. Backward compatibility is maintained for older backup formats

## Backup During Onboarding

When setting up a new SQLite Muninn instance, the onboarding flow includes a **Restore from SQLite backup** step. You can upload a backup ZIP directly during onboarding to skip manual setup.
